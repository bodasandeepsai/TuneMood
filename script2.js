    ...
    if (!recognition || recognition.status === 'inactive') {
        ...
        recognition.onresult = (event) => {
            ...
            if (command === 'show my playlist') {
                ...
                addToPlaylist(tracks[currentIndex]);
...
function getCurrentTrack() {
    const songNameElement = document.querySelector('.minimusicplayer .songname');
    if (!songNameElement) {
        return null;
    }
    ...
    return tracks.find(track => track.title === title && track.artist.name === artist);
}
